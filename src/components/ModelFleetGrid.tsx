...
const MODEL_FLEET: ModelInfo[] = [
  { name: 'Claude Opus 4.6', desc: 'Primary research & orchestration', icon: '🔴', color: 'var(--model-opus)', agents: ['Muddy', 'Elon', 'Gary', 'Warren', 'Scribe', 'Pixel', 'Deal', 'Closer'], primary: true },
  ...
  return (
    <div className="mb-10">
      <div className="flex items-center gap-3 mb-5">
        <h2 className="text-lg font-semibold" style={{ color: 'var(--text-primary)' }}>🤖 Model Fleet</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {MODEL_FLEET.map(m => {
          ...
          const borderColor = m.primary ? 'var(--accent-gold)' : m.color + '33'
          return (
            <div
              key={m.name}
              className="rounded-xl p-5 hover:border-[var(--accent-teal)] transition-colors cursor-default"
              style={{
                background: 'var(--bg-card)',
                border: `1px solid ${borderColor}`,
                boxShadow: m.primary ? '0 0 12px rgba(255,215,0,0.08)' : 'none',
              }}
            >
              <div className="flex items-start justify-between mb-3">
                ...
                <span className="flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full font-medium shrink-0" style={{
                  ...
                  <span className="w-1.5 h-1.5 rounded-full" style={{ background: stats.status === 'Active' ? 'var(--accent-green)' : 'var(--text-secondary)' }}></span>
                  {stats.status}
                </span>
              </div>
              {/* Agent names */}
