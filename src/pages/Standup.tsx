...
const SPEAKER_COLORS: Record<string, { color: string; role: string }> = {
  ...
  'Steve Jobs': { color: '#FF9800', role: 'CMO' },
  'Marc Benioff': { color: '#7B1FA2', role: 'CRO' },
}
    ...
    <>
      ...
      <div className="rounded-lg p-5 mb-6" style={{ background: 'var(--bg-2)', border: '1px solid var(--border-divider)' }}>
        <div className="flex items-center gap-2 mb-1">
          <span className="text-base font-semibold flex-1">{standup.title}</span>
          <span className="text-xs px-2 py-1 rounded" style={{ background: 'var(--bg-3)', color: 'var(--text-secondary)', border: '1px solid var(--border-divider)' }}>Demo</span>
        </div>
